...
/**
 * 根据实例的json数据生成带默认实例化的echarts配置。同时开放数据源接口，方便数据的对接
 *  1. 从 http://echarts.baidu.com/examples.html 上得到模板文件
 *  2. 配置数据源 标记 DataSource
 *  3. 生成
 */
public class ChartsGenerator {

    private static Logger logger = LoggerFactory.getLogger(ChartsGenerator.class);
    ...
    public JSONObject loadSchemaJson(String schemaPath) throws Exception{
        URL schemaUrl = ChartsGenerator.class.getResource(schemaPath);
        if (schemaUrl == null) {
