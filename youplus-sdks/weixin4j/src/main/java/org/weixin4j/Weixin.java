...
public class Weixin extends WeixinSupport implements java.io.Serializable {
    ...
    public User getUserInfo(String openId, String lang) throws WeixinException {
        ...
        HttpsClient http = new HttpsClient();
        //调用获取access_token接口
        Response res = http.get("https://api.weixin.qq.com/cgi-bin/test/info" + param);
        //根据请求结果判定，是否验证成功
        JSONObject jsonObj = res.asJSONObject();
        if (jsonObj != null) {
            if (Configuration.isDebug()) {
    ...
    public Followers getUserList(String next_openid) throws WeixinException {
        ...
        HttpsClient http = new HttpsClient();
        //调用获取access_token接口
        Response res = http.get("https://api.weixin.qq.com/cgi-bin/test/get" + param);
        //根据请求结果判定，是否验证成功
        JSONObject jsonObj = res.asJSONObject();
    ...
    public String uploadMediaByUrl(MediaType mediaType, String urlStr, String fileName) throws WeixinException {
        //创建请求对象
        HttpsClient http = new HttpsClient();
        //上传素材，返回JSON数据包
        String jsonStr = http.uploadHttpsByUrl("https://api.weixin.qq.com/cgi-bin/media/upload?access_token=" + this.oauthToken.getAccess_token() + "&type=" + mediaType.toString(), urlStr, fileName);
        JSONObject jsonObj = JSONObject.parseObject(jsonStr);
        if (jsonObj != null) {
            if (Configuration.isDebug()) {
                System.out.println("上传多媒体文件返回json：" + jsonObj.toString());
    ...
    public Followers getTagUserList(int tagid, String next_openid) throws WeixinException {
        ...
        HttpsClient http = new HttpsClient();
        //调用获取标签下粉丝列表接口
        Response res = http.get("https://api.weixin.qq.com/cgi-bin/test/tag/get" + param);
        //根据请求结果判定，是否验证成功
        JSONObject jsonObj = res.asJSONObject();
        Followers follower = null;
        if (jsonObj != null) {
    ...
    public void updateUserInfoRemark(String openId, String remark) throws WeixinException {
        ...
        HttpsClient http = new HttpsClient();
        //调用获取access_token接口
        Response res = http.post("https://api.weixin.qq.com/cgi-bin/test/info/updateremark?access_token=" + this.oauthToken.getAccess_token(), postParam);
        //根据请求结果判定，是否验证成功
        JSONObject jsonObj = res.asJSONObject();
        if (jsonObj != null) {
            if (Configuration.isDebug()) {
