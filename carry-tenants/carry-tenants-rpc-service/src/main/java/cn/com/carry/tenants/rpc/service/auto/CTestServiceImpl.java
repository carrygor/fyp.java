package cn.com.carry.tenants.rpc.service.auto;

import cn.com.carry.model.auto.entity.tenants.CTest;
import cn.com.carry.tenants.dao.auto.CTestMapper;
import cn.com.carry.tenants.api.auto.CTestService;
import cn.com.carry.common.util.base.MybatisServiceImpl;
import org.springframework.stereotype.Service;

/**
 * <p>
 *  服务实现类
 * </p>
 *
 * @author 何文浩
 */
@Service("cTestService")
public class CTestServiceImpl extends MybatisServiceImpl<CTestMapper, CTest> implements CTestService {
	
}
